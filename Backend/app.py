...
class SmallGPT(tf.keras.Model):
    ...
    def get_config(self):
        config = super().get_config()
        config.update({"sequence_length": self.sequence_length, "vocab_size": self.vocab_size,
...
MODEL.fit(_X_train_lm, _y_train_lm,
...
def predict():
    """
    Body JSON : { "prompt": "naruto is", "num_words": 5 }
    Retourne  : voir generate_with_explanation()
    """
    if "user" not in session:
    ...
    if not prompt:
        return jsonify(error=True, message="Prompt vide.")
